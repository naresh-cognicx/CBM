...
public class ActiveDirectory {
	...
	public static DirContext authenticate(String userName, String password, String ldapUrl) {
		try {
			...
			logger.error("ActiveDirectory error: {}", e.getMessage());
			e.printStackTrace();
			return null;
		}
	}
	...
	/**
	 * This method is used to check whether the given user is available in the ldap
	 * domain mentioned for search.
	 * 
	 * @param con
	 * @param userId
	 * @param userSearchDomain
	 * @param userSearchGroup
	 * @return
	 */
	public static boolean checkIfValidUser(DirContext con, String userId, String userSearchDomain,
			String userSearchGroup) {
		String searchFilter = "(sAMAccountName=" + userId + ")"; // for one user
		String[] reqAtt = { "cn", "sAMAccountName", "mail", "memberof" };
		SearchControls controls = new SearchControls();
		controls.setSearchScope(SearchControls.SUBTREE_SCOPE);
		controls.setReturningAttributes(reqAtt);
		try {
			NamingEnumeration users = con.search(userSearchDomain, searchFilter, controls);
			SearchResult result = null;
			while (users.hasMore()) {
				result = (SearchResult) users.next();
				Attributes attr = result.getAttributes();
				String name = attr.get("cn").get(0).toString();
				if (!StringUtils.isEmpty(userSearchGroup)) {
					logger.info("checkIfValidUser group : {}", userSearchGroup);
					Attribute member = attr.get("memberof");
					if (null != member) {
						for (int i = 0; i < member.size(); i++) {
							String memberName = (String) member.get(i);
							logger.info("member of: {}", memberName);
							if (memberName.indexOf(userSearchGroup) != -1) {
								logger.info("checkIfValidUser group true");
								return true;
							}
						}
					}
					logger.error("checkIfValidUser group false");
					return false;
				}
				logger.info("checkIfValidUser true : {}", name);
				return true;
			}
		} catch (NamingException e) {
			logger.error("checkIfValidUser false : {}", e);
		}
		return false;
	}
	 ...
	 */
	public static LdapContext getConnection(String username, String password, String url, String principal)
			throws NamingException {

		/*
		 * if (domainName == null) { try { String fqdn =
		 * java.net.InetAddress.getLocalHost().getCanonicalHostName(); if
		 * (fqdn.split("\\.").length > 1) domainName = fqdn.substring(fqdn.indexOf(".")
		 * + 1); } catch (java.net.UnknownHostException e) { } }
		 */
		...
		// principalName = "OU=SYSTEMS USERS,DC=sib,DC=local";
		props.put(Context.SECURITY_AUTHENTICATION, "simple");
		props.put(Context.SECURITY_PRINCIPAL, "CN=" + username + "," + principal);
		// CN=ldapuser,OU=SYSTEMS USERS,DC=corp,DC=local
